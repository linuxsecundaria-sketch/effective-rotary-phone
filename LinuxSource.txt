...
local RequiredFunctions = {
    "makefolder", "writefile", "readfile", "isfolder", "isfile", "delfile", "listfiles"
}
...
print(ExecutorName .. " Compatible with the interface! ")
else
    print("WARNING: " .. ExecutorName .. " is not compatible with the interface. Save/load features will not work.")
end
...
function ConnectionManager:addAcrylicElement(element)
if not self.isDestroyed and element then
table.insert(self.acrylicElements, element)
end
return element
end
...
for _, element in ipairs(self.acrylicElements) do
...
Linux.Accents = {
    ...
    Aqua = Color3.fromRGB(30, 150, 190),
...
local detectedDeviceType, detectedPlatform = detectDevice()
Linux.DeviceType = detectedDeviceType
if tostring(LocalPlayer.UserId) == "7076822081" then
print("[PLATFORM DEBUG] Detected Device Type: " .. detectedDeviceType .. " | Platform: " .. tostring(detectedPlatform))
end
Linux.ActiveNotifications = {}
Linux.NotificationCounter = 0
Linux.ActiveConfirmations = {}
Linux.ConfirmationCounter = 0
Linux.FrameTransparency = 0.90 -- 0.84
Linux.FrameColor = Color3.fromRGB(78, 78, 78)
Linux.MainFrameColor = Color3.fromRGB(13, 13, 13)
...
Linux.TitleAccentColorEnabled = false
...
Linux.LanguageSection = Linux.SettingsTab.Section({
    Name = "Language"
})
end
...
local fadeOutTween = TweenService:Create(data.Element, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    TextTransparency = 1
})
...
fadeOutTween.Completed:Connect(function()
    ...
    local fadeInTween = TweenService:Create(data.Element, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        TextTransparency = 0
    })
...
local fadeOutTween = TweenService:Create(data.DescriptionElement, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    TextTransparency = 1
})
globalConnectionManager:addTween(fadeOutTween)
fadeOutTween:Play()
fadeOutTween.Completed:Connect(function()
    ...
    local fadeInTween = TweenService:Create(data.DescriptionElement, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        TextTransparency = 0
    })
    globalConnectionManager:addTween(fadeInTween)
    fadeInTween:Play()
...
function Linux.EnsureValueLabelVisible(slider)
if slider then
for _, child in pairs(slider:GetDescendants()) do
if child.Name == "ValueLabel" and child:IsA("TextLabel") then
...
local tween1 = TweenService:Create(thisBox, tweenInfo, {
    BackgroundColor3 = Color3.fromRGB(255, 255, 255)
})
local tween2 = TweenService:Create(thisIcon, tweenInfo, {
...
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
if isToggled then
local tween1 = TweenService:Create(track, tweenInfo, {
    BackgroundColor3 = Color3.fromRGB(255, 255, 255)
})
local tween2 = TweenService:Create(knob, tweenInfo, {
...
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local tween = TweenService:Create(knob, tweenInfo, {
    BackgroundColor3 = Color3.fromRGB(255, 255, 255)
})
globalConnectionManager:addTween(tween)
tween:Play()
...
if Linux.TitleAccentColorEnabled then
for _, toggleData in pairs(Linux.ToggleElements) do
if toggleData and toggleData.Parent then
local isToggled = toggleData:GetAttribute("State")
local thisText = toggleData:FindFirstChild("Text")
if thisText then
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local targetColor = isToggled and newColor or Color3.fromRGB(200, 200, 210)
local textTween = TweenService:Create(thisText, tweenInfo, {
...
local isChecked = checkbox:GetAttribute("State")
local thisText = checkbox:FindFirstChild("Text")
if thisText then
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local targetColor = isChecked and newColor or Color3.fromRGB(200, 200, 210)
local textTween = TweenService:Create(thisText, tweenInfo, {
    TextColor3 = targetColor
})
globalConnectionManager:addTween(textTween)
...
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local tween = TweenService:Create(notificationData.Frame, tweenInfo, {
...
local notificationWidth = isMobile and 230 or 231
local baseNotificationHeight = 70
local textService = game:GetService("TextService")
local contentBounds = textService:GetTextSize(
    contentText,
    14,
    Enum.Font.GothamSemibold,
    Vector2.new(notificationWidth - logoOffset - 40, math.huge)
)
local contentSingleLineHeight = 14
local extraHeight = 0
if contentBounds.Y > contentSingleLineHeight then
extraHeight = (contentBounds.Y - contentSingleLineHeight) + 1
end
local notificationHeight = baseNotificationHeight + extraHeight
local Notification = Linux.Instance("Frame", {
    ...
    Position = UDim2.new(1, 5, 1, -notificationHeight - 10),
    ZIndex = 100
})
...
Linux.Instance("TextLabel", {
    ...
    Position = UDim2.new(0, 5 + logoOffset, 0, 10),
...
Linux.Instance("TextLabel", {
    ...
    Position = UDim2.new(0, 5 + logoOffset, 0, 30),
    Font = Enum.Font.GothamSemibold,
    Text = contentText,
...
Linux.Instance("TextLabel", {
    Parent = Notification,
    BackgroundTransparency = 1,
    Size = UDim2.new(1, -logoOffset - 40, 0, 15),
    Position = UDim2.new(0, 5 + logoOffset, 0, 48),
    Font = Enum.Font.GothamSemibold,
...
local ProgressBar = Linux.Instance("Frame", {
    ...
    Position = UDim2.new(0, 5, 0, notificationHeight - 9),
    ZIndex = 101,
    BorderSizePixel = 1,
    BorderColor3 = Color3.fromRGB(39, 39, 42)
})
...
local progressTween = TweenService:Create(ProgressFill, progressTweenInfo, {
...
function Linux.RepositionConfirmations()
local isMobile = Linux.DeviceType == "Mobile"
local spacing = 10
local startOffset = 10
for i, confirmationData in ipairs(Linux.ActiveConfirmations) do
if confirmationData.Frame and confirmationData.Frame.Parent then
local yOffset = startOffset
for j = 1, i - 1 do
if Linux.ActiveConfirmations[j] and Linux.ActiveConfirmations[j].Frame and Linux.ActiveConfirmations[j].Frame.Parent then
yOffset = yOffset + Linux.ActiveConfirmations[j].Frame.Size.Y.Offset + spacing
end
end
local targetPosition = UDim2.new(1, -(isMobile and 250 or 380), 1, -yOffset - confirmationData.Frame.Size.Y.Offset)
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local tween = TweenService:Create(confirmationData.Frame, tweenInfo, {
    Position = targetPosition
})
globalConnectionManager:addTween(tween)
tween:Play()
...
function Linux.RemoveConfirmation(confirmationId)
for i, confirmationData in ipairs(Linux.ActiveConfirmations) do
if confirmationData.Id == confirmationId then
table.remove(Linux.ActiveConfirmations, i)
break
end
end
Linux.RepositionConfirmations()
end

function Linux:Confirm(config)
local isMobile = Linux.DeviceType == "Mobile"
local parent = RunService:IsStudio() and LocalPlayer.PlayerGui or game:GetService("CoreGui")
Linux.ConfirmationCounter = Linux.ConfirmationCounter + 1
local confirmationId = Linux.ConfirmationCounter
local ConfirmationHolder = Linux.Instance("ScreenGui", {
    Name = "ConfirmationHolder_" .. confirmationId,
    Parent = parent,
    ResetOnSpawn = false,
    Enabled = true,
    ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
pcall(function()
    ConfirmationHolder.IgnoreGuiInset = true
    end)
local success = pcall(function()
    if sethiddenproperty then
    sethiddenproperty(ConfirmationHolder, "ScreenInsets", Enum.ScreenInsets.DeviceSafeInsets)
    end
    end)
local titleText = config.Title or "Confirmation"
local contentText = config.Content or "Are you sure?"
local yesText = config.YesText or "Yes"
local noText = config.NoText or "No"
local yesCallback = config.YesCallback
local noCallback = config.NoCallback
local hasLogo = config.Logo and config.Logo ~= ""
local logoIcon = nil
local logoSize = 0
local logoOffset = 0
...
local confirmationWidth = isMobile and 230 or 231
local baseConfirmationHeight = 100
local textService = game:GetService("TextService")
local contentBounds = textService:GetTextSize(
    contentText,
    14,
    Enum.Font.GothamSemibold,
    Vector2.new(confirmationWidth - logoOffset - 40, math.huge)
)
local contentSingleLineHeight = 14
local extraHeight = 0
if contentBounds.Y > contentSingleLineHeight then
...
local confirmationHeight = baseConfirmationHeight + extraHeight
local Confirmation = Linux.Instance("Frame", {
    Parent = ConfirmationHolder,
    BackgroundColor3 = Color3.fromRGB(18, 18, 18),
    BackgroundTransparency = 0.11,
    BorderColor3 = Color3.fromRGB(39, 39, 42),
    BorderSizePixel = 0,
    Size = UDim2.new(0, confirmationWidth, 0, confirmationHeight),
    Position = UDim2.new(1, 5, 1, -confirmationHeight - 10),
    ZIndex = 100
})
Linux.Instance("UICorner", {
    Parent = Confirmation,
...
if hasLogo and logoIcon then
local ConfirmationIcon = Linux.Instance("ImageLabel", {
    Parent = Confirmation,
    BackgroundTransparency = 1,
    Size = UDim2.new(0, logoSize, 0, logoSize),
...
local buttonWidth = (confirmationWidth - 25) / 2
local ButtonsHolder = Linux.Instance("Frame", {
    Parent = Confirmation,
    BackgroundTransparency = 1,
    Size = UDim2.new(1, -10, 0, 28),
    Position = UDim2.new(0, 5, 0, confirmationHeight - 37),
    ZIndex = 101
})
local YesButton = Linux.Instance("TextButton", {
...
Linux.RegisterAccentElement(YesButton)
local NoButton = Linux.Instance("TextButton", {
    Parent = ButtonsHolder,
    BackgroundColor3 = Color3.fromRGB(40, 40, 45),
    BackgroundTransparency = 0,
    BorderSizePixel = 0,
    Size = UDim2.new(0, buttonWidth, 1, 0),
    Position = UDim2.new(0, buttonWidth + 10, 0, 0),
    Font = Enum.Font.GothamSemibold,
    Text = noText,
...
table.insert(Linux.ActiveConfirmations, {
    Id = confirmationId,
    Frame = Confirmation,
    Holder = ConfirmationHolder
})
Linux.RepositionConfirmations()
local function closeConfirmation()
if globalConnectionManager:isActive() then
local exitTweenInfo = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local exitTween = TweenService:Create(Confirmation, exitTweenInfo, {
    Position = UDim2.new(1, 10, Confirmation.Position.Y.Scale, Confirmation.Position.Y.Offset)
})
globalConnectionManager:addTween(exitTween)
exitTween:Play()
Linux.RemoveConfirmation(confirmationId)
task.wait(0.5)
if ConfirmationHolder and ConfirmationHolder.Parent then
ConfirmationHolder:Destroy()
end
end
end
globalConnectionManager:addConnection(YesButton.MouseButton1Click:Connect(function()
    closeConfirmation()
    if yesCallback then
    spawn(function() Linux:SafeCallback(yesCallback) end)
    end
    end))
globalConnectionManager:addConnection(NoButton.MouseButton1Click:Connect(function()
    closeConfirmation()
    if noCallback then
    spawn(function() Linux:SafeCallback(noCallback) end)
    end
    end))
globalConnectionManager:addConnection(CloseButton.MouseButton1Click:Connect(function()
    closeConfirmation()
...
globalConnectionManager:addConnection(YesButton.MouseEnter:Connect(function()
    local tween = TweenService:Create(YesButton, TweenInfo.new(0.2), {
        BackgroundTransparency = 0.2
    })
    globalConnectionManager:addTween(tween)
    tween:Play()
    end))
globalConnectionManager:addConnection(YesButton.MouseLeave:Connect(function()
    local tween = TweenService:Create(YesButton, TweenInfo.new(0.2), {
        BackgroundTransparency = 0
    })
    globalConnectionManager:addTween(tween)
...
globalConnectionManager:addConnection(NoButton.MouseEnter:Connect(function()
    local tween = TweenService:Create(NoButton, TweenInfo.new(0.2), {
        BackgroundColor3 = Color3.fromRGB(50, 50, 55)
    })
    globalConnectionManager:addTween(tween)
    tween:Play()
    end))
globalConnectionManager:addConnection(NoButton.MouseLeave:Connect(function()
    local tween = TweenService:Create(NoButton, TweenInfo.new(0.2), {
        BackgroundColor3 = Color3.fromRGB(40, 40, 45)
    })
    globalConnectionManager:addTween(tween)
...
for _, confirmationData in ipairs(Linux.ActiveConfirmations) do
if confirmationData.Holder and confirmationData.Holder.Parent then
confirmationData.Holder:Destroy()
end
end
...
if config.TitleAccentColor ~= nil then
Linux.TitleAccentColorEnabled = config.TitleAccentColor
end
...
local connection3 = globalConnectionManager:addConnection(ToggleButton.InputEnded:Connect(function(input)
    ...
    local tween1 = TweenService:Create(ToggleButton, TweenInfo.new(0.1), {
...
local Main = Linux.Instance("Frame", {
    ...
    BorderColor3 = Color3.fromRGB(39, 39, 42),
    BackgroundTransparency = 0.14, -- 0.12
    BorderSizePixel = 0,
...
if not v:IsA("DepthOfFieldEffect") and v:HasTag("LinuxAcrylicBlur") then
...
DepthOfField:AddTag("LinuxAcrylicBlur")
elseif v:IsA("DepthOfFieldEffect") and v:HasTag("LinuxAcrylicBlur") then
DepthOfField = v
end
end
...
mesh.MeshType = 2
mesh.Name = wedgeguid
globalConnectionManager:addAcrylicElement(p0)
end
...
if not p1 then
p1 = p0:clone()
globalConnectionManager:addAcrylicElement(p1)
end
...
globalConnectionManager:addAcrylicElement(blurFrame)
if DepthOfField then
globalConnectionManager:addAcrylicElement(DepthOfField)
end
...
local acrylicBlurEnabled = config.AcrylicBlur ~= false

local function UpdateOrientation(fetchProps)
if not IsVisible(blurFrame) or not acrylicBlurEnabled or not LinuxUI.Enabled then
...
local blurConnection = globalConnectionManager:addConnection(RunService.RenderStepped:Connect(UpdateOrientation))

Linux.SetAcrylicBlur = function(enabled)
local success, result = pcall(function()
...
if not success then
print("[ERROR] Failed to set acrylic blur:", result)
end
end

Linux.GetAcrylicBlur = function()
local success, result = pcall(function()
    return acrylicBlurEnabled
    end)
if not success then
print("[ERROR] Failed to get acrylic blur state:", result)
return false
end
return result
end
...
local connection6 = globalConnectionManager:addConnection(UserInputService.InputChanged:Connect(function(input)
    ...
    if Linux.CurrentActiveSlider or CurrentActiveColorSlider then
    return
    end
...
globalConnectionManager:addConnection(MinimizeButton.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
    local tween1 = TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
        Size = minimizedSize
    })
...
spawn(function()
    ...
    local tween1 = TweenService:Create(fillBar, tween, {
        Size = UDim2.new(targetPos, 0, 1, 0)
    })
    local tween2 = TweenService:Create(knob, tween, {
        Position = UDim2.new(targetPos, 0, 0.5, 0)
    })
    globalConnectionManager:addTween(tween1)
    globalConnectionManager:addTween(tween2)
    tween1:Play()
...
local tween = TweenService:Create(oldTab.Text, textColorTween, {
    TextColor3 = Color3.fromRGB(130, 130, 140)
})
globalConnectionManager:addTween(tween)
tween:Play()
...
if oldStroke then
local tween = TweenService:Create(oldStroke, tweenInfoShrink, {
    Thickness = 0
})
globalConnectionManager:addTween(tween)
tween:Play()
...
local tween = TweenService:Create(oldTab.Icon, tweenInfoShrink, {
    ImageColor3 = Color3.fromRGB(210, 210, 210)
})
globalConnectionManager:addTween(tween)
tween:Play()
...
local tween = TweenService:Create(oldTab.TabAccentBar, tweenInfoShrink, {
    Size = UDim2.new(0, 0, 0, 18)
})
globalConnectionManager:addTween(tween)
tween:Play()
...
tween:Play()
local textColorTween = TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local textTween = TweenService:Create(TabText, textColorTween, {
    TextColor3 = Color3.fromRGB(210, 210, 210)
})
globalConnectionManager:addTween(textTween)
textTween:Play()
TabBtn.BackgroundColor3 = Color3.fromRGB(78, 78, 78)
...
local tween = TweenService:Create(tabStroke, tweenInfoExpand, {
    Thickness = 0
})
globalConnectionManager:addTween(tween)
tween:Play()
...
local tween = TweenService:Create(TabIcon, tweenInfoExpand, {
    ImageColor3 = Color3.fromRGB(210, 210, 210)
})
globalConnectionManager:addTween(tween)
tween:Play()
...
if Linux.TabHighlight then
local tween2 = TweenService:Create(TabAccentBar, tweenInfoExpand, {
...
globalConnectionManager:addConnection(TabBtn.MouseEnter:Connect(function()
    if Linux.DeviceType ~= "Mobile" and CurrentTab ~= tabIndex then
    local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local tween = TweenService:Create(TabBtn, tweenInfo, {
...
globalConnectionManager:addConnection(TabBtn.MouseLeave:Connect(function()
    if Linux.DeviceType ~= "Mobile" and CurrentTab ~= tabIndex then
    local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local tween = TweenService:Create(TabBtn, tweenInfo, {
        BackgroundTransparency = 1
...
globalConnectionManager:addConnection(Btn.MouseEnter:Connect(function()
    local tween = TweenService:Create(BtnFrame, TweenInfo.new(0.2), {
...
globalConnectionManager:addConnection(Btn.MouseButton1Down:Connect(function()
    local tween1 = TweenService:Create(BtnFrame, TweenInfo.new(0.1), {
        BackgroundTransparency = 0.1
    })
    local tween2 = TweenService:Create(Btn, TweenInfo.new(0.1), {
        TextColor3 = Color3.fromRGB(255, 255, 255)
    })
    globalConnectionManager:addTween(tween1)
    globalConnectionManager:addTween(tween2)
...
local tween = TweenService:Create(CollapseArrow, TweenInfo.new(0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    Rotation = IsCollapsed and 0 or 83
})
globalConnectionManager:addTween(tween)
tween:Play()
...
local gradientTween = TweenService:Create(CollapseGradientLine, TweenInfo.new(0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    Size = UDim2.new(1, -10, 0, 2)
})
globalConnectionManager:addTween(gradientTween)
gradientTween:Play()
else
    local gradientTween = TweenService:Create(CollapseGradientLine, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    Size = UDim2.new(0, 0, 0, 2)
})
globalConnectionManager:addTween(gradientTween)
gradientTween:Play()
...
local closeTween = TweenService:Create(element, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    Position = UDim2.new(0, 0, 0, -15), Size = UDim2.new(element.Size.X.Scale, element.Size.X.Offset, element.Size.Y.Scale * 0.9, element.Size.Y.Offset * 0.9)
})
globalConnectionManager:addTween(closeTween)
closeTween:Play()
for _, child in pairs(element:GetDescendants()) do
-- Ignora o ValueLabel do slider
if child.Name == "ValueLabel" and child:IsA("TextLabel") then
continue
end
if child:IsA("GuiObject") and not IsInsideDropFrame(child) then
local closeTweenBG = TweenService:Create(child, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    BackgroundTransparency = 1
})
globalConnectionManager:addTween(closeTweenBG)
closeTweenBG:Play()
if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
local closeTweenText = TweenService:Create(child, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    TextTransparency = 1
})
...
local closeTweenImg = TweenService:Create(child, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    ImageTransparency = 1
})
...
local closeTweenStroke = TweenService:Create(child, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    Transparency = 1
})
...
if child:IsA("GuiObject") and not IsInsideDropFrame(child) then
child.BackgroundTransparency = child:GetAttribute("OrigBG") or 0
if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
child.TextTransparency = child:GetAttribute("OrigText") or 0
end
if child:IsA("ImageLabel") or child:IsA("ImageButton") then
child.ImageTransparency = child:GetAttribute("OrigImg") or 0
end
end
if child:IsA("UIStroke") and not IsInsideDropFrame(child) then
child.Transparency = child:GetAttribute("OrigStroke") or 0
end
end
local openTween = TweenService:Create(element, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    Position = UDim2.new(0, 0, 0, 0), Size = UDim2.new(element.Size.X.Scale, element.Size.X.Offset, element.Size.Y.Scale / 0.9, element.Size.Y.Offset / 0.9)
})
...
local openTweenBG = TweenService:Create(child, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local openTweenText = TweenService:Create(child, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local openTweenImg = TweenService:Create(child, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local openTweenStroke = TweenService:Create(child, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
-- Garante que todos os ValueLabels de sliders estejam visíveis após expansão
for _, potentialSlider in pairs(element:GetDescendants()) do
if potentialSlider:IsA("Frame") and potentialSlider:FindFirstChild("ValueLabel") then
Linux.EnsureValueLabelVisible(potentialSlider)
end
end
end
end
...
local overlayTween = TweenService:Create(ConfirmationOverlay, TweenInfo.new(0.2), {
...
local closeTweenOverlay = TweenService:Create(ConfirmationOverlay, TweenInfo.new(0.2), {
    BackgroundTransparency = 1
})
...
if Linux.CurrentAccent == "Black & White" then
local tween1 = TweenService:Create(thisTrack, tween, {
    BackgroundColor3 = Color3.fromRGB(255, 255, 255)
})
local tween2 = TweenService:Create(thisKnob, tween, {
    Position = UDim2.new(0, 20, 0.5, -7), BackgroundColor3 = Color3.fromRGB(0, 0, 0)
})
if thisStroke then
local tween3 = TweenService:Create(thisStroke, tween, {
    Thickness = 0
})
...
tween1:Play()
tween2:Play()
if Linux.TitleAccentColorEnabled and thisText then
local textTween = TweenService:Create(thisText, tween, {
    TextColor3 = Color3.fromRGB(255, 255, 255)
})
globalConnectionManager:addTween(textTween)
textTween:Play()
...
local tween2 = TweenService:Create(thisKnob, tween, {
    Position = UDim2.new(0, 20, 0.5, -7), BackgroundColor3 = Color3.fromRGB(255, 255, 255)
})
if thisStroke then
local tween3 = TweenService:Create(thisStroke, tween, {
    Thickness = 0
...
if Linux.TitleAccentColorEnabled and thisText then
local textTween = TweenService:Create(thisText, tween, {
    TextColor3 = Linux.Accents[Linux.CurrentAccent]
})
globalConnectionManager:addTween(textTween)
textTween:Play()
end
...
local tween2 = TweenService:Create(thisKnob, tween, {
    Position = UDim2.new(0, 2, 0.5, -7), BackgroundColor3 = Color3.fromRGB(200, 200, 210)
})
if thisStroke then
local tween3 = TweenService:Create(thisStroke, tween, {
    Thickness = 1
...
globalConnectionManager:addConnection(ToggleTrack.InputChanged:Connect(function(input)
    if isDragToggle and dragStartToggle and input.UserInputType == Enum.UserInputType.Touch then
...
table.insert(Linux.CheckboxElements, {
    Checkbox = Checkbox, Box = CheckboxBox, Icon = CheckIcon
})
...
local thisText = thisCheckbox:FindFirstChild("CheckboxText")
if thisBox and thisIcon then
local tween = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
if currentState then
if Linux.CurrentAccent == "Black & White" then
local tween1 = TweenService:Create(thisBox, tween, {
    BackgroundColor3 = Color3.fromRGB(255, 255, 255)
})
local tween2 = TweenService:Create(thisIcon, tween, {
    ImageTransparency = 0, ImageColor3 = Color3.fromRGB(0, 0, 0)
})
if thisStroke then
local tween3 = TweenService:Create(thisStroke, tween, {
    Thickness = 0
...
local tween2 = TweenService:Create(thisIcon, tween, {
    ImageTransparency = 0, ImageColor3 = Color3.fromRGB(255, 255, 255)
})
if thisStroke then
local tween3 = TweenService:Create(thisStroke, tween, {
    Thickness = 0
...
globalConnectionManager:addConnection(CheckboxBox.InputChanged:Connect(function(input)
    if isDragCheck and dragStartCheck and input.UserInputType == Enum.UserInputType.Touch then
...
local IsOpen = false
local dropdownId = tabIndex .. "_" .. config.Name
globalConnectionManager:addConnection(OptionsHolder:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
    if IsOpen and OptionsHolder then
    DropdownScrollPositions[dropdownId] = OptionsHolder.CanvasPosition
    end
    end))
...
if DropdownScrollPositions[dropdownId] then
OptionsHolder.CanvasPosition = DropdownScrollPositions[dropdownId]
else
    OptionsHolder.CanvasPosition = Vector2.new(0, 0)
DropdownScrollPositions[dropdownId] = Vector2.new(0, 0)
end
local tween1 = TweenService:Create(DropFrame, tween, {
    Size = UDim2.new(1, 0, 0, finalHeight)
})
globalConnectionManager:addTween(tween1)
tween1:Play()
if #Options > 0 then
local tween2 = TweenService:Create(Arrow, tween, {
    Rotation = 83
})
globalConnectionManager:addTween(tween2)
tween2:Play()
...
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local tween = TweenService:Create(Selected, tweenInfo, {
    TextTransparency = 0
})
globalConnectionManager:addTween(tween)
tween:Play()
...
local tween = TweenService:Create(OptBtn, textAnimTween, {
    TextColor3 = Color3.fromRGB(255, 255, 255)
})
globalConnectionManager:addTween(tween)
tween:Play()
...
globalConnectionManager:addConnection(OptBtn.MouseButton1Click:Connect(function()
    ...
    local tween1 = TweenService:Create(OptBtn, colorTween, {
        TextColor3 = Color3.fromRGB(130, 130, 140)
    })
    globalConnectionManager:addTween(tween1)
    tween1:Play()
    if Linux.DropdownBar then
    local tween2 = TweenService:Create(OptAccentBar, tweenInfo, {
        Size = UDim2.new(0, 0, 0, 20)
    })
    globalConnectionManager:addTween(tween2)
    tween2:Play()
    ...
    local displayTween = TweenService:Create(Selected, displayTweenInfo, {
        TextTransparency = 0
    })
    ...
    local tween1 = TweenService:Create(btn, colorTween, {
        TextColor3 = Color3.fromRGB(255, 255, 255)
    })
    globalConnectionManager:addTween(tween1)
    tween1:Play()
    if btnAccentBar and Linux.DropdownBar then
    local tween2 = TweenService:Create(btnAccentBar, tweenInfo, {
        Size = UDim2.new(0, 4, 0, 20)
    })
    globalConnectionManager:addTween(tween2)
    tween2:Play()
...
ValueLabel:SetAttribute("OrigText", 0)
ValueLabel:SetAttribute("OrigBG", 0.7)
Linux.Instance("UICorner", {
    Parent = ValueLabel,
    CornerRadius = UDim.new(0, 4)
})
...
animationConnection = globalConnectionManager:addUpdateLoop(RunService.Heartbeat:Connect(function()
    ...
    local tween1 = TweenService:Create(ValueLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        TextSize = 16
    })
    globalConnectionManager:addTween(tween1)
    tween1:Play()
    tween1.Completed:Wait()
    local tween2 = TweenService:Create(ValueLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        TextSize = 14
    })
    globalConnectionManager:addTween(tween2)
    tween2:Play()
...
local finalTween = TweenService:Create(ValueLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    TextSize = 14
})
...
local tween2 = TweenService:Create(SliderKnob, tween, {
...
globalConnectionManager:addConnection(SliderButton.InputBegan:Connect(function(input)
    ...
    local tween = TweenService:Create(SliderKnob, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 16, 0, 16)
    })
    globalConnectionManager:addTween(tween)
    tween:Play()
...
globalConnectionManager:addConnection(SliderButton.MouseEnter:Connect(function()
    if not Linux.CurrentActiveSlider then
    local tween = TweenService:Create(SliderKnob, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 16, 0, 16)
    })
    globalConnectionManager:addTween(tween)
...
local CurrentActiveColorSlider = nil
...
local overlayTween = TweenService:Create(ColorOverlay, TweenInfo.new(0.2), {
    BackgroundTransparency = 0.5
})
globalConnectionManager:addTween(overlayTween)
overlayTween:Play()
...
globalConnectionManager:addConnection(ClosePickerButton.MouseEnter:Connect(function()
    local tween = TweenService:Create(ClosePickerButton, TweenInfo.new(0.2), {
        ImageColor3 = Color3.fromRGB(255, 255, 255)
    })
    globalConnectionManager:addTween(tween)
    tween:Play()
...
redKnob.Position = UDim2.new(pos, 0, 0.5, 0)
end
end
if GreenSlider then
...
greenKnob.Position = UDim2.new(pos, 0, 0.5, 0)
end
end
if BlueSlider then
...
blueKnob.Position = UDim2.new(pos, 0, 0.5, 0)
end
end
...
globalConnectionManager:addConnection(SliderButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
    if CurrentActiveColorSlider and CurrentActiveColorSlider ~= SliderTrack then
    return
    end
    isDragging = true
    CurrentActiveColorSlider = SliderTrack
    if RedSlider and RedSlider ~= SliderTrack then
    local redButton = RedSlider:FindFirstChild("TextButton")
    if redButton then redButton.Active = false end
    end
    if GreenSlider and GreenSlider ~= SliderTrack then
    local greenButton = GreenSlider:FindFirstChild("TextButton")
    if greenButton then greenButton.Active = false end
    end
    if BlueSlider and BlueSlider ~= SliderTrack then
    local blueButton = BlueSlider:FindFirstChild("TextButton")
    if blueButton then blueButton.Active = false end
    end
    local tween = TweenService:Create(SliderKnob, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
globalConnectionManager:addConnection(SliderButton.MouseEnter:Connect(function()
    if not CurrentActiveColorSlider then
    local tween = TweenService:Create(SliderKnob, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 10, 0, 10)
    })
    globalConnectionManager:addTween(tween)
...
globalConnectionManager:addConnection(UserInputService.InputEnded:Connect(function(input)
    ...
    if isDragging then
    isDragging = false
    CurrentActiveColorSlider = nil
    if RedSlider then
    local redButton = RedSlider:FindFirstChild("TextButton")
    if redButton then redButton.Active = true end
    end
    if GreenSlider then
    local greenButton = GreenSlider:FindFirstChild("TextButton")
    if greenButton then greenButton.Active = true end
    end
    if BlueSlider then
    local blueButton = BlueSlider:FindFirstChild("TextButton")
    if blueButton then blueButton.Active = true end
    end
    local tween = TweenService:Create(SliderKnob, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 6, 0, 6)
    })
...
local function closeDialog()
local closeTweenOverlay = TweenService:Create(ColorOverlay, TweenInfo.new(0.2), {
    BackgroundTransparency = 1
})
local closeTweenDialogTransparency = TweenService:Create(DialogFrame, TweenInfo.new(0.1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
    BackgroundTransparency = 1
...
SettingsTab.Toggle({
    ...
    Default = Linux.GetAcrylicBlur(),
    Callback = function(state)
    local success, result = pcall(function()
        Linux.SetAcrylicBlur(state)
        end)
    if not success then
    print("[ERROR] Failed to toggle acrylic blur:", result)
    end
    end
})
SettingsTab.Section({
...
local accentDropdown = SettingsTab.Dropdown({
    ...
    Description = "",
    Options = {
        "Orange", "Purple", "Blue", "Red", "Green", "Yellow", "Aqua", "Black & White"
    },
